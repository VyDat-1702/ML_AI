...
def Forwardpass(X, Wh1, bh1, Wh2, bh2, Wo, bo):
    zh1 = np.matmul(X, Wh1.T) + bh1 
    a1 = ReLU(zh1)        
    zh2 = np.matmul(a1, Wh2.T) + bh2 
    a2 = sigmoid(zh2)     
    z =np.matmul(a2, Wo.T) + bo  
    o = softmax(z)         
...
# Train function
def train(x_train, y_train, x_test, y_test, train_samples, batch_size, Wh1, bh1, Wh2, bh2, Wo, bo, epochs, lr):
    loss_history = []
    acc_history = []
    
    print("\n-----Neural Network Mini-Batch GD Summary-----")
    ...
    print(f"\tParameters: {Wh1_init.size + bh1_init.size}")
    print("Hidden Layer 2:")
    print(f"\tActivation Shape: ({miniBatch_size}, {h2_units})")
    print(f"\tActivation Size: {miniBatch_size * h2_units}")
    print(f"\tParameters: {Wh2_init.size + bh2_init.size}")
    print("Output Layer:")
    print(f"\tActivation Shape: ({miniBatch_size}, {num_classes})")
    print(f"\tActivation Size: {miniBatch_size * num_classes}")
    print(f"\tParameters: {Wo_init.size + bo_init.size}")
    print(f"Total parameter: {Wh1_init.size + bh1_init.size + Wh2_init.size + bh2_init.size + Wo_init.size + bo_init.size}\n")
    ...
    for ep in range(1, epochs + 1):
        ...
        for ite in range(0, train_samples, batch_size):
            ...
            # Forward pass
            zh1, a1, zh2, a2, o = Forwardpass(x, Wh1, bh1, Wh2, bh2, Wo, bo)
            ...
            dbh2 = np.mean(dh2s, axis=0)
            dWh1 = np.matmul(dh1s.T, x)
            dbh1 = np.mean(dh1s, axis=0)
        ...
        # Compute accuracy
        _, _, _, _, prediction = Forwardpass(x_test, Wh1, bh1, Wh2, bh2, Wo, bo)
        acc = AccTest(y_test, prediction)
        acc_history.append(acc)
        
        if ep % 10 == 0:
            print(f"Epoch {ep}/{epochs} - Loss: {loss_history[ep-1]:.4f} - Accuracy: {acc_history[ep-1]:.4f}")
    
    return Wh1, bh1, Wh2, bh2, Wo, bo, loss_history, acc_history

# Train
Wh1, bh1, Wh2, bh2, Wo, bo, loss_history, acc_history = train(
    x_train, y_train, x_test, y_test, train_samples, miniBatch_size,
    Wh1_init, bh1_init, Wh2_init, bh2_init, Wo_init, bo_init, epochs, lr
)

#Evaluate
_, _, _, _, o = Forwardpass(x_test, Wh1, bh1, Wh2, bh2, Wo, bo)
